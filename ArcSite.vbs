...
'Do any form initialization in this sub
  ...
  If objEditForm.Mode = 3 Then
    ...
    'Get the ArcSite layer's recordset (to pass into the ReturnNextID function)
    Dim objArcSiteRS
    Set objArcSiteRS = Layer.Records
    'Update the Site ID
    objEFPageOneControls("txtRSITEID").Value = ReturnNextID (objArcSiteRS, "RSITEID")
  End If
...
Sub ValidateRWMVAR
'Validate the Raw Material Variability field.  Must contain a 
'value or prompt to enter one.
  Dim strEnteredValue
  strEnteredValue = CStr(ThisEvent.Object.Value)
  If strEnteredValue = Empty Then
    ThisEvent.Result = False
    ThisEvent.MessageText = "What is the raw material variability?"
    ThisEvent.MessageType = vbExclamation
  End If
End Sub
...
Sub ValidateImplement
'Validate the all implements recorded Variability field.  Must contain a 
'value or prompt to enter one.
  Dim strEnteredValue
  strEnteredValue = CStr(ThisEvent.Object.Value)
  If strEnteredValue = Empty Then
    ThisEvent.Result = False
    ThisEvent.MessageText = "Are all implements recorded?"
    ThisEvent.MessageType = vbExclamation
  End If
End Sub

Sub ValidateAdjacentSwale
'Validate the Adjacent Swale Variability field.  Must contain a 
'value or prompt to enter one.
  Dim strEnteredValue
  strEnteredValue = CStr(ThisEvent.Object.Value)
  If strEnteredValue = Empty Then
    ThisEvent.Result = False
    ThisEvent.MessageText = "What's in the adjacent swale?"
    ThisEvent.MessageType = vbExclamation
  End If
End Sub